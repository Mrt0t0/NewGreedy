...
from urllib.parse import urlparse, parse_qs, urlencode, unquote_to_bytes
...
def _extract_infohash(query_string):
    """
    Extrait info_hash depuis la query string brute (bytes ou str).
    Retourne un hex string 8-40 chars, propre et stable.
    """
    ...
    for part in query_string.split(b"&"):
        if part.startswith(b"info_hash="):
            ...
            try:
                ...
                try:
                    return raw.decode("latin-1").encode("latin-1").hex()
                except Exception:
                    return raw.hex()
    return ""
...
class NewGreedyAddon:
    ...
    def _load_stats(self):
        ...
        try:
            ...
            for k, d in raw.items():
                k = k.lower().strip()
                if not VALID.match(k): continue
                if not isinstance(d, dict): continue
                # Accepter v1.4 (rep_ul) et v1.5 (cumul_rep_ul)
    ...
    def _save_stats(self):
        try:
            data = {
                ih: {
                    "cumul_rep_ul": s._cumul_rep_ul, "cumul_rep_dl": s._cumul_rep_dl,
                    "cumul_real_ul": s._cumul_real_ul, "ann_count": s._ann_count,
                    "stalled": s._is_stalled, "prev_rep_ul": s._prev_rep_ul,
                }
    ...
    def request(self, flow: http.HTTPFlow):
        ...
        # parse_qs pour les autres paramètres
        parsed = urlparse(url)
        ...
        if self._spoof_pid:
            if ih_key not in self._peer_ids:
                pid = _rand_peer_id()
                self._peer_ids[ih_key] = pid; self._uas[ih_key] = _ua_for_prefix(pid[:8])
        ...
        if self._spoof_peers:
            ...
            qs["numwant"] = [str(int(nw*random.uniform(0.85,1.15)))]

        if corrupt_val is not None: qs["corrupt"] = [str(corrupt_val)]
        if random.random()<self._event_anom_p and event not in ("started","stopped"):
            qs["event"] = ["started"]

        res = struct.pack(">I", random.randint(1,4096))
        qs["uploaded"]   = [str(int(new_ul))]
        qs["downloaded"] = [str(int(new_dl + res[0]))]
        flow.request.path = parsed.path + "?" + urlencode({k:v[0] for k,v in qs.items()})
        ...
        cum_dl   = st._cumul_rep_dl/1e6; rul_mb = real_ul/1e6
        cum_ul   = new_ul/1e6; delta_mb = delta_ul/1e6
        ...
        eta      = int(max(0,target_ul-new_ul)/avg_d) if avg_d>0 else 0
        ...
        if st._cumul_rep_dl > 0:
            logger.info("[%s] %s | DL:%8.2fMB | RealUL:%8.2fMB | CumUL:%8.2fMB | +DUL:%7.2fMB | SentUL:%8.2fMB | Ratio:%.3f ETA:~%dann | Ann#%d%s%s",
                mode, ih_key, cum_dl, rul_mb, cum_ul, delta_mb, cum_ul, ratio, eta, st._ann_count, stag_t, stall_t)
        else:
            logger.info("[%s] %s | DL:%8.2fMB | RealUL:%8.2fMB | CumUL:%8.2fMB | +DUL:%7.2fMB | SeedUL:%8.2fMB | Ann#%d%s%s",
                mode, ih_key, cum_dl, rul_mb, cum_ul, delta_mb, cum_ul, st._ann_count, stag_t, stall_t)

        # Sauvegarder dès ann#1 puis toutes les 5
